...
class _CouponsCardDetailsState extends State<CouponsCardDetails> {
  ...
  Future<Map<int, int>> _fetchAllCouponUsage() async {
    ...
    for (var coupon in couponList) {
      int usage = await service.fetchCouponUsage(coupon.couponNo);
  ...
  Future<Map<int, String>> _fetchAllCouponStatus() async {
    ...
    for (var coupon in couponList) {
      String status =
          await service.checkCouponStatus(auth.userId, coupon.couponNo);
                                      ...
                                      Row(
                                        children: [
                                          GestureDetector(
                                            onTap: () {
                                              _sendEmail(
                                                  'Check out this coupons',
                                                  '${coupon.couponDesc.toString()}\n discount: ${coupon.savings}%');
                                            },
                                            child: Icon(
                                              Icons.email,
                                              size: 10.w,
